...
class ModelDistributionNetwork(nn.Module):
    ...
    def compute_loss(self, images, actions, step_types, step=None, rewards=None, discounts=None, latent_posterior_samples_and_dists=None):
        ...
        # ------------------------------------------------------------------ recon term
        preds_imgs   = self.decoder(z1_post, z2_post).clamp(0,1)            # p(x|z)  Independent Normal
        ...
        axs[0].imshow(fd_true[0, 0,0].cpu().detach().numpy(), cmap="gray")
        axs[1].imshow(mask[0,0,0].cpu().detach().numpy(), cmap="inferno")
        axs[2].imshow(fd_pred[0, 0,0].cpu().detach().numpy(), cmap="gray")
        ...
        # 2. L1 loss on the differences
        lambda_fd = 30.0                     # tweak 0.02-0.1 if needed
        fd_loss = lambda_fd * (mask * (fd_pred - motion).abs()).sum() \
                     / mask.sum().clamp(min=1)
        
        """BT, _, H, W = preds_imgs.shape[0]*preds_imgs.shape[1], 1, preds_imgs.shape[3], preds_imgs.shape[4]
        edges_pred = sobel(preds_imgs.view(-1,1,H,W)).view_as(preds_imgs)
        edges_true = sobel(images.view(-1,1,H,W)).view_as(images)
        edge_loss  = 0.05 * F.l1_loss(edges_pred, edges_true)"""
    ...
    def visual_diagnostics(self, seq, actions, step_types, H=7):
        ...
        z1, z2  = self.sample_posterior(seq, actions, step_types)[0]    # (1,L,d1/2)
...
# Sobel kernels once globally
def sobel(x):
    gx = F.conv2d(x, sobel_kx.to(x.device), padding=1)
    gy = F.conv2d(x, sobel_ky.to(x.device), padding=1)

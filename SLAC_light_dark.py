...
@torch.no_grad()
def polyak_update_(target: torch.nn.Module, online: torch.nn.Module, tau: float):
    # target ← (1−tau) target + tau online
    for p_t, p in zip(target.parameters(), online.parameters()):
        p_t.data.mul_(1.0 - tau).add_(p.data, alpha=tau)

    # also copy buffers (e.g. running stats if any)
    for b_t, b in zip(target.buffers(), online.buffers()):
        b_t.copy_(b)
...
if __name__ == '__main__':
    ...
    TargetModel = copy.deepcopy(Model).to(args.device)
    TargetModel.eval()
    TargetModel.requires_grad_(False)
    ...
    if args.from_scratch:
        ...
        if args.save_model:
            path = f"checkpoints\\LightDarkNavigation\\{run_name}\\model_pretrained_kl_teacher_random.pth"
    ...
    with torch.no_grad():
        ...
        feat0  = TargetModel.encoder(imgs0)                           # (N, feat)
        z1_bel = TargetModel.latent1_first_posterior(feat0).rsample() # (N, d1)
        z2_bel = TargetModel.latent2_first_posterior(z1_bel).rsample()# (N, d2)
    ...
    for global_step in tqdm(range(args.total_timesteps+1)):
     ...
     # -------- Bayes filter: PREDICT (use PRIORS) --------
        with torch.no_grad():
            ...
            p1     = TargetModel.latent1_prior(z2_bel, a_one).base_dist
            z1_prd = p1.loc  # mean prediction (lower variance than sampling)
            # p(z^2_t | z^1_t, z^2_{t-1}, a_{t-1})
            p2     = TargetModel.latent2_prior(z1_prd, z2_bel, a_one).base_dist
            z2_prd = p2.loc
        
        # -------- Bayes filter: UPDATE (use POSTERIORS with current frame) --------
        with torch.no_grad():
            imgs = torch.from_numpy(obs).reshape(1,1,-1).to(device).float() / world_radius
            feat = TargetModel.encoder(imgs)  # (N, feat)
            # q(z^1_t | x_t, z^2_{t-1}, a_{t-1})
            q1   = TargetModel.latent1_posterior(feat, z2_bel, a_one)
            z1_t = q1.rsample()
            # q(z^2_t | z^1_t, z^2_{t-1}, a_{t-1})
            q2   = TargetModel.latent2_posterior(z1_t, z2_bel, a_one)
            z2_t = q2.rsample()
        ...
        if global_step > args.learning_starts and global_step % args.train_frequency == 0:            
            ...
            with torch.no_grad():
                (z1, z2), _ = TargetModel.sample_posterior(images, actions[:, :-1], step_ty)
            ...
            polyak_update_(TargetModel, Model, tau=args.tau)
